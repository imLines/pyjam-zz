...
import DemonCard from "../../partials/cards/DemonCard";
import SortBarSexyBodysuit from "../../partials/sortbar/SortBarSexyBodysuit";

function SexyBodysuit(){
        ...
        return (
            <>
                <section className="flex flex-col items-center bg-p-light min-h-screen pt-5">
                    <h1 className="text-2xl font-[900] mb-16 flex"><img className="w-8 h-8" src={demonIcon} alt="demon icon" />Nuisettes<img className="w-8 h-8" src={demonIcon} alt="demon icon" /></h1>
                    ...
                    />
                    <button onClick={changeVisibilitySortBar} className={sortBarOpen ? 'hidden' : "border shadow-sm shadow-p-purple  border-p-purple rounded-md px-3 py-2 text-p-purple cursor-pointer hover:bg-p-purple hover:text-p-light active:shadow-none  transition ease-in-out delay-75 my-4"}>Trier</button>
                    <div className=" flex flex-row flex-wrap justify-center items-center w-full mx-auto md:flex-row md:flex-wrap md:justify-center">
                        ...
                        {customSearchActive ? customSearchArray?.map(item => <DemonCard item={item} key={item.id} />) : allProducts?.map(item => <DemonCard item={item} key={item.id} />)}
                    </div>
