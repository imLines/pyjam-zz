...
function DemonCard({ item }) {
    ...
    if (loading) {
        return (
            <>
                <Link className="shadow-md shadow-p-black  h-56 w-56 mx-9  mb-6 md:w-96 md:h-72  rounded-lg relative  flex items-end bg-gray-200 animate-pulse">
        ...
        return (
            <>
                <Link to={`/client/produit/${item.id}`} className={`shadow-md shadow-p-demon-dark  h-56 w-56 mx-9  mb-6 md:w-96 md:h-72  rounded-lg relative  flex items-end`}>
                    <img src={picture} alt="" className="h-56 w-56  md:w-96 md:h-72  rounded-lg object-cover" />
                    <div className=" absolute bottom-0 left-0 right-0 w-full h-1/3 bg-p-demon-dark/70 rounded-b-lg p-2 flex flex-col justify-between">
